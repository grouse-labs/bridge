...
local function get_player(player_src)
  ...
  return qbx:GetPlayer(player_src)
end
...
local function add_money(player, money_type, amount)
  ...
  local prev = qbx:GetMoney(player, money_type)
  qbx:AddMoney(money_type:lower(), amount, 'bridge added money: '..amount)
...
local function remove_money(player, money_type, amount)
  ...
  local prev = qbx:GetMoney(player, money_type)
  qbx:RemoveMoney(money_type:lower(), amount, 'bridge removed money: '..amount)
  return qbx:GetMoney(player, money_type) == prev - amount
end
...
local function get_inventory(player)
  ...
  local PlayerItems = ox_inventory:GetInventoryItems(player --[[@as string|integer]])
  if not PlayerItems then error('error calling \'getplayerinventory\' (inventory not found)', 2) end
  for name, slot in pairs(PlayerItems) do
    inventory[name] = {
      name = slot.name,
      label = slot.label,
      weight = slot.weight,
...
local function has_item(player, item_name, amount)
  ...
  return ox_inventory:GetItemCount(player --[[@as string|integer]], item_name) >= (amount or 1)
end
...
local function add_item(player, item_name, amount)
  ...
  return ox_inventory:CanCarryItem(player --[[@as string|integer]], item_name, amount or 1) and ox_inventory:AddItem(player --[[@as string|integer]], item_name, amount or 1) or false
end
