...
local function register_callback(name, cb)
  ...
  RegisterNetEvent(CB_EVENT:format(name), function(resource, key, ...)
    TriggerServerEvent(CB_EVENT:format(resource), key, receive_callback(pcall(cb, ...)))
  end)
end
...
local function trigger_callback(name, cb, ...)
  ...
  TriggerServerEvent(string.format(CB_EVENT, name), RES_NAME, key, ...)
...
local function await_callback(name, ...)
  ...
  return trigger_callback(name, nil, ...)
end
