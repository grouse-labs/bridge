...
function notify.text(player, text, _type, time)
  ...
  TriggerClientEvent('QBCore:Notify', player, text, _type, time)
end
...
function notify.item(player, item, amount, text)
  ...
  if not item or type(item) ~= 'string' then error('bad argument #2 to \'item\' (string expected, got '..type(item)..')', 2) end
  ...
  amount = amount or 1
  TriggerClientEvent('qb-inventory:client:ItemBox', player, QBCore.Shared.Items[item:lower()], amount >= 1 and 'add' or 'remove', math.abs(amount))
